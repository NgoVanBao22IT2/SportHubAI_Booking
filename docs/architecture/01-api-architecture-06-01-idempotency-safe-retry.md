...
- **Phân Phân Tích Kiến Trúc Tạo Đơn Đặt Sân (`POST /api/v1/bookings`):**
  ...
  3. Nếu Client gặp sự cố mạng và retry gửi lại `POST /api/v1/bookings` với cùng `Idempotency-Key`:
     ...
     - Backend trả lại thông tin đơn hàng đã giữ chỗ (`status: HOLDING`) mà **KHÔNG tạo thêm đơn giữ chỗ thứ hai** và **KHÔNG báo lỗi đúp slot `BOOKING_SLOT_OCCUPIED` cho chính Client đó**.
